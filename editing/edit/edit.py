...
from configs.path_conf import edit_paths, model_paths
from editing.edit.sketchedit import Styleedit
from editing.edit.model import Generator


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--exp", type=str, default="./experiment",
                        help="Path to inference results with `latents.npy` saved here.")
    parser.add_argument("--weight_deltas_path", type=str, default="./weight_deltas",
                        help="Root path holding all weight deltas.")
    ...
    parser.add_argument("--delta_i_c", type=str, default=edit_paths["edit"]["delta_i_c"],
                        help="path to file containing delta_i_c")
    parser.add_argument("--s_statistics", type=str, default=edit_paths["edit"]["s_statistics"],
                        help="path to file containing s statistics")
    parser.add_argument("--text_prompt_templates", default=edit_paths["edit"]["templates"])
...
def load_direction_calculator(args):
    ...
    global_direction_calculator = Styleedit(delta_i_c, s_std, text_prompt_templates)
...
def run():
    ...
    latents = np.load(os.path.join(args.exp, 'latents.npy'), allow_pickle=True).item()
    # prepare output directory
    args.output_path = os.path.join(args.exp, "styleclip_edits", f"{args.neutral_text}_to_{args.target_text}")
