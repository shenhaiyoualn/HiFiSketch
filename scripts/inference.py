...
def run():
    ...
    out_path_results = os.path.join(test_opts.exp, 'inference_results')
    out_path_coupled = os.path.join(test_opts.exp, 'inference_coupled')
    ...
    dataset_args = data_conf.DATASETS[opts.dataset_type]
    ...
    for input_batch in tqdm(dataloader):
        ...
        for i in range(input_batch.shape[0]):
            ...
            if opts.save_weight_deltas:
                weight_deltas_dir = os.path.join(test_opts.exp, "weight_deltas")
    ...
    stats_path = os.path.join(opts.exp, 'stats.txt')
    ...
    np.save(os.path.join(test_opts.exp, 'latents.npy'), all_latents)
