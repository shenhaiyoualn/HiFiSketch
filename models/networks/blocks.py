...
class HifiREBlock(Module):
    def __init__(self, hifinet, n_channels=512, inner_c=128, spatial=16):
        super(HifiREBlock, self).__init__()
        self.n_channels = n_channels
        self.inner_c = inner_c
        self.out_c = 512
        ...
        self.hifinet = hifinet

    def forward(self, features):
        code = self.convs(features)
        code = code.view(-1, self.out_c)
        code = self.linear(code)
        weight_delta = self.hifinet(code)
        return weight_delta


class ReBlockSeparable(Module):

    def __init__(self, layer_idx, opts, n_channels=512, inner_c=256, spatial=16):
        super(ReBlockSeparable, self).__init__()
        self.layer_idx = layer_idx
        self.kernel_size, self.in_channels, self.out_channels = PARAMETERS[self.layer_idx]
        self.spatial = spatial
        self.n_channels = n_channels
