...
from models.networks.blocks import HifiREBlock, RefinementBlock, ReBlockSeparable
from models.networks.SWhifinetwork import SharedWeightsHifinet


class Hifinet(Module):
    ...
    def __init__(self, opts):
        ...
        if len(opts.layers_to_tune) == 0:
            self.layers_to_tune = list(range(opts.n_hifinet_outputs))
        else:
            self.layers_to_tune = [int(l) for l in opts.layers_to_tune.split(',')]

        self.shared_layers = [0, 2, 3, 5, 6, 8, 9, 11, 12]
        self.shared_weight_hifinet = SharedWeightsHifinet(in_size=512, out_size=512, mode=None)

        self.refinement_blocks = nn.ModuleList()
        self.n_outputs = opts.n_hifinet_outputs
        for layer_idx in range(self.n_outputs):
            if layer_idx in self.layers_to_tune:
                if layer_idx in self.shared_layers:
                    refinement_block = HifiREBlock(self.shared_weight_hifinet, n_channels=512, inner_c=128)
                else:
...
class Hifinets(Module):

    def __init__(self, opts):
        super(Hifinets, self).__init__()
        ...
        for layer_idx in range(self.n_outputs):
            if layer_idx in self.layers_to_tune:
                if layer_idx in self.shared_layers:
                    ...
                    refinement_block = ReBlockSeparable(layer_idx, opts, n_channels=512, inner_c=256)
            else:
                refinement_block = None
            self.refinement_blocks.append(refinement_block)
