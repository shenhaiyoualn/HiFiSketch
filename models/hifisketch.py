...
from models.networks.hifinetwork import Hifinet, Hifinets
from utils.resnet_mapping import RESNET_MAPPING


class Hifisketch(nn.Module):

    def __init__(self, opts):
        super(Hifisketch, self).__init__()
        self.set_opts(opts)
        self.n_styles = int(math.log(self.opts.output_size, 2)) * 2 - 2
        # Define architecture
        self.hifinet = self.set_hifinet()
    ...
    def set_hifinet(self):
        if self.opts.output_size == 1024:
            self.opts.n_hifinet_outputs = 26
        elif self.opts.output_size == 512:
            self.opts.n_hifinet_outputs = 23
        elif self.opts.output_size == 256:
            self.opts.n_hifinet_outputs = 20
        else:
            raise ValueError(f"Invalid Output Size! Support sizes: [1024, 512, 256]!")
        networks = {
            "hifinet": Hifinet(opts=self.opts),
            "hifinets": Hifinets(opts=self.opts),
        }
        return networks[self.opts.encoder_type]

    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            print(f'Loading Hifisketch from checkpoint: {self.opts.checkpoint_path}')
            ckpt = torch.load(self.opts.checkpoint_path, map_location='cpu')
            self.hifinet.load_state_dict(self.__get_keys(ckpt, 'hifinet'), strict=True)
            self.decoder.load_state_dict(self.__get_keys(ckpt, 'decoder'), strict=True)
            self.__load_latent_avg(ckpt)
            if self.opts.load_w_encoder:
                self.w_encoder = self.__get_pretrained_w_encoder()
        else:
            hifinet_ckpt = self.__get_hifinet_checkpoint()
            self.hifinet.load_state_dict(hifinet_ckpt, strict=False)
    ...
    def forward(self, x, resize=True, input_code=False, randomize_noise=True, return_latents=False,
        ...
        if input_code:
            ...
            hifinet_outputs = self.hifinet(x_input)
            if weights_deltas is None:
                weights_deltas = hifinet_outputs
            else:
                weights_deltas = [weights_deltas[i] + hifinet_outputs[i] if weights_deltas[i] is not None else None
                                  for i in range(len(hifinet_outputs))]
    ...
    def __get_hifinet_checkpoint(self):
        print('Loading hifinet weights!')
        hifinet_ckpt = torch.load(model_paths['resnet34'])

        if self.opts.input_nc != 3:
            shape = hifinet_ckpt['conv1.weight'].shape
            altered_input_layer = torch.randn(shape[0], self.opts.input_nc, shape[2], shape[3], dtype=torch.float32)
            altered_input_layer[:, :3, :, :] = hifinet_ckpt['conv1.weight']
            hifinet_ckpt['conv1.weight'] = altered_input_layer
        mapped_hifinet_ckpt = dict(hifinet_ckpt)
        for p, v in hifinet_ckpt.items():
            for original_name, net_name in RESNET_MAPPING.items():
                if original_name in p:
                    mapped_hifinet_ckpt[p.replace(original_name, net_name)] = v
                    mapped_hifinet_ckpt.pop(p)
        return hifinet_ckpt
